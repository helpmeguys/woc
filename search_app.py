...
# === CONFIGURATION (Must be before set_page_config) ===
...
def generate_login_token(password_input):
    return hashlib.sha256(password_input.encode()).hexdigest()

expected_token = generate_login_token(PASSWORD)
if st.session_state.login_token == expected_token:
...
if not st.session_state.authenticated:
    ...
    if password_input:
        if verify_password(password_input):
            st.session_state.authenticated = True
            st.session_state.login_token = generate_login_token(password_input)
            log_access()
