...
# === BAR GRAPH OF EVENTS PER MONTH ===
st.subheader("📊 Event Frequency Overview")

# Prepare data for graph
monthly_summary = {
    month: sum(events.values())
    for month, events in monthly_counts.items()
}
monthly_df = pd.DataFrame.from_dict(monthly_summary, orient='index', columns=["Event Count"])
...
if activity_data:
    csv_df = pd.DataFrame(activity_data)
    csv = csv_df.to_csv(index=False).encode("utf-8")
    st.download_button(
        label="📥 Download Full Activity Log as CSV",
        data=csv,
        file_name="activity_log.csv",
        mime="text/csv"
    )
else:
    st.info("No activity log data available to export.")
