...
if not query:
    ...
    with st.spinner("Searching by meaning..."):
        ...
        for idx, (sim, qa) in enumerate(top_results):
            ...
            if f"listen_{idx}" not in st.session_state:
                st.session_state[f"listen_{idx}"] = False
            ...
            with st.container():
                ...
                with col3:
                    button_text = "🔊 Hide audio" if st.session_state[f"listen_{idx}"] else "🎧 Listen"
                    if st.button(button_text, key=f"listen_btn_{idx}"):
                        st.session_state[f"listen_{idx}"] = not st.session_state[f"listen_{idx}"]

            st.markdown(f"🔍 <span style='color:green;'>Semantic similarity: {sim:.3f}</span>", unsafe_allow_html=True)

            if st.session_state[f"listen_{idx}"]:
