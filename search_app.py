...
if not query:
    ...
    with st.spinner("Searching by meaning..."):
        ...
        for idx, (sim, qa) in enumerate(top_results):
            ...
            if listen_key not in st.session_state:
                st.session_state[listen_key] = False
            ...
            with st.container():
                ...
                with col2:
                    mailto = f"mailto:?subject={urllib.parse.quote(SITE_TITLE)}&body={urllib.parse.quote(f'Question: {question}
...
These results were rendered at https://search.thewordsofchrist.org')}"
                    ...
                    st.markdown(f"<a href='{mailto}' target='_blank'><button style='width:100%'>📧 Send email</button></a>", unsafe_allow_html=True)

                with col3:
                    button_key = f"listen_btn_{idx}"
                    button_text = "🔊 Hide audio" if st.session_state[listen_key] else "🎧 Listen"
                    if st.button(button_text, key=button_key):
                        st.session_state[listen_key] = not st.session_state[listen_key]

            st.markdown(f"🔍 <span style='color:green;'>Semantic similarity: {sim:.3f}</span>", unsafe_allow_html=True)

            if st.session_state[listen_key]:
