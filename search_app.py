...
def add_bookmark(user_id, question, answer):
    conn = get_connection()
    c = conn.cursor()
    c.execute("INSERT INTO bookmarks (user_id, question, answer, timestamp) VALUES (?, ?, ?, ?)",
...
def get_bookmarks(user_id):
    conn = get_connection()
    c = conn.cursor()
    c.execute("SELECT id, question, answer, timestamp FROM bookmarks WHERE user_id = ? ORDER BY timestamp DESC", (user_id,))
    results = c.fetchall()
    conn.close()
    return results
...
Watch here: {url}"
            ...
            with col3:
                if st.button("⭐ Bookmark", key=f"bookmark_{idx}"):
                    add_bookmark(st.session_state.user_id, question, answer)
        ...
        for b_id, q, a, ts in bookmarks:
            with st.container():
                ...
                if st.button("❌ Delete", key=f"del_{b_id}"):
                    ...
                    st.experimental_rerun()
                st.markdown("---")
        df = pd.DataFrame(bookmarks, columns=["ID", "Question", "Answer", "Timestamp"])
        csv = df.to_csv(index=False).encode('utf-8')
        st.download_button("📥 Download Bookmarks as CSV", data=csv, file_name="my_bookmarks.csv", mime="text/csv")
    else:
        st.info("No bookmarks yet. Start asking questions and bookmarking answers!")st.markdown("---")
st.subheader("⭐ My Bookmarks")
bookmarks = get_bookmarks(st.session_state.user_id)
if bookmarks:
    for b_id, q, a, ts in bookmarks:
