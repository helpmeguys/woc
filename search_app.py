# gospel_search_app_with_admin.py (finalized and cleaned)
...
import matplotlib.pyplot as pltimport pandas as pd# Modify your log_event function to include the datedef log_event(event_type, data=None):    event = {        "event": event_type,        "date": datetime.now().date().isoformat(),  # Added date key        "timestamp": datetime.now().isoformat(),    }    if data:        event.update(data)    try:        with open(ACTIVITY_LOG_FILE, "r") as f:            logs = json.load(f)    except FileNotFoundError:        logs = []    logs.append(event)    with open(ACTIVITY_LOG_FILE, "w") as f:        json.dump(logs, f, indent=2)# Admin panel graph and CSV downloadif query_page == "admin":    st.title("📊 Admin Dashboard")    if not st.session_state.admin_authenticated:        # Authentication block unchanged        pass    # Load and prepare data    try:        with open(ACTIVITY_LOG_FILE, "r") as f:            activity_data = json.load(f)    except FileNotFoundError:        activity_data = []    # Organize data by date    daily_counts = defaultdict(lambda: Counter())    for log in activity_data:        daily_counts[log["date"]][log["event"]] += 1    # Prepare data for plotting and CSV    df = pd.DataFrame([{**{'date': date}, **counts} for date, counts in daily_counts.items()])    df['date'] = pd.to_datetime(df['date'])    df.sort_values('date', inplace=True)    df.set_index('date', inplace=True)    # Plotting    st.subheader("Daily Activity Overview")    fig, ax = plt.subplots()    for event in ['open_clicks', 'emails_sent', 'searches']:  # Adjust event labels as needed        if event in df.columns:            ax.plot(df.index, df[event], label=event.replace('_', ' ').title())    ax.legend()    ax.set_xlabel('Date')    ax.set_ylabel('Count')    ax.set_title('Daily Event Counts')    st.pyplot(fig)    # CSV Download    csv = df.to_csv(index=True)    st.download_button(label="Download data as CSV", data=csv, file_name='daily_activity.csv', mime='text/csv')    # Other parts of admin panel remain unchanged    st.markdown("---")    st.markdown("[🔙 Back to App](./)")    st.stop()if query_page == "admin":
...
# === MAIN UI ===
st.markdown(f"<h3 style='margin-bottom: 0.5rem;'>📖 {SITE_TITLE}</h3>", unsafe_allow_html=True)
st.markdown("Ask a gospel question and receive spiritually insightful answers, matched by meaning.")
            ...
            with st.container():
                ...
                with col2:
                    if st.button("📧 Send email", key=f"email_{idx}"):
                        ...
                        mailto = f"mailto:?subject={urllib.parse.quote('Your Gospel Q&A Answer')}&body={urllib.parse.quote('Question: ' + question + '\n\nAnswer: ' + answer + '\n\nWatch here: ' + url + '\n\nThese results were rendered at https://search.thewordsofchrist.org')}"
