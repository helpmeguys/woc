...
if query_page == "admin":
    ...
    st.subheader("📅 Daily Activity Chart")
    ...
    with st.expander("📆 Filter by Date Range"):
        ...
        start_date = st.date_input("Start date", min_value=min_date, max_value=max_date, value=date.today().replace(day=1))
                end_date = st.date_input("End date", min_value=min_date, max_value=max_date, value=date.today())

    filtered_df = df[(df['date'] >= start_date) & (df['date'] <= end_date)]
    
    daily_counts = filtered_df.groupby(['date', 'event']).size().reset_index(name='count')(chart, use_container_width=True)

    st.subheader("⬇️ Download Activity Logs")
