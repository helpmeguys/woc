...
if not query:
    ...
    with st.spinner("Searching by meaning..."):
        ...
        for idx, (sim, qa) in enumerate(top_results):
            ...
            with col1:
                if st.button("📋 Copy", key=f"copy_{idx}", help="Copy video link to clipboard"):
            ...
            with col2:
                if st.button("📧 Email", key=f"email_{idx}", help="Send question and answer via email"):
                    ...
                    st.write(f"<script>window.open('{mailto}', '_blank');</script>", unsafe_allow_html=True)
            with col3:
                if st.button("🎧 Listen", key=f"listen_{idx}", help="Play audio version of the answer"):
                    st.session_state[f"listen_{idx}_active"] = True
                    log_event("audio_played", {"question": question})

            if st.session_state.get(f"listen_{idx}_active"):
                with st.expander("🎰 Listen to this answer", expanded=True):
                    audio_key = f"audio_{idx}"
                    if audio_key not in st.session_state:
                        st.session_state[audio_key] = generate_tts_audio(f"Question: {question}. Answer: {answer}")
