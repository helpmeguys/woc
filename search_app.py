...
if not st.session_state.authenticated:
    st.markdown(f"""
        <div style='padding-top: 0rem; margin-top: .5rem;'>
            <h3 style='margin-bottom: 0.5rem;'>🔐 {SITE_TITLE}</h3>
            <p>This tool is available <strong>free to registered users.</strong></p>
            <p>➡️ <a href="{REGISTRATION_URL}" target="_blank">Click here to register</a> and receive your access password.</p>
        </div>
    ...
    if password == PASSWORD:
        st.session_state.authenticated = True  # ✅ Will persist even after refresh
        log_access()
...
# === MAIN UI ===
st.markdown(f"""
    <div style='margin-top: .5rem; padding-bottom: 1rem;'>
        <h3 style='margin-bottom: 0.5rem;'>📖 {SITE_TITLE}</h3>
    </div>
""", unsafe_allow_html=True)
