# gospel_search_app_with_admin.py (updated for working logging)
        ...
        for idx, (sim, qa) in enumerate(top_results):
            ...
            with col1:
                if st.button("📋 Copy link", key=f"copy_{idx}"):
                    ...
                    st.write(f"[Copied]({url})")  # dummy trigger

            with col2:
                if st.button("📧 Send email", key=f"email_{idx}"):
                    log_event("email_sent", {"url": url})
                    mailto = f"mailto:?subject={urllib.parse.quote('Your Gospel Q&A Answer')}&body={urllib.parse.quote(f'Question: {question}\n\nAnswer: {answer}\n\nWatch here: {url}\n\nThese results were rendered at https://search.thewordsofchrist.org')}"
                    js = f"window.location.href='{mailto}'"
