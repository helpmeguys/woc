            ...
            components.html(f"""
                <div style='margin-top:4px; display: flex; gap: 8px;'>
                    <button id="copy-btn-{idx}" onclick="navigator.clipboard.writeText('{url}');
                        ...
                        fetch('/log_link_click_{idx}');"
                        style="cursor:pointer; padding:4px 10px; font-size:0.85rem; border:1px solid #ccc; border-radius:5px; background:#f9f9f9;">
                        📋 Copy link
                    </button>
                    <button id="email-btn-{idx}" onclick="
                        const btn = document.getElementById('email-btn-{idx}');
                        btn.innerText='📧 Emailing...';
                        setTimeout(() => btn.innerText='📧 Send email', 2000);
                        window.location.href='mailto:?subject={urllib.parse.quote("Your Gospel Q&A Answer")}&body={urllib.parse.quote(f"Question: {question}\n\nAnswer: {answer}\n\nWatch here: {url}\n\nThese results were rendered at https://search.thewordsofchrist.org")}';
                        fetch('/log_email_click_{idx}');"
                        style="cursor:pointer; padding:4px 10px; font-size:0.85rem; border:1px solid #ccc; border-radius:5px; background:#f9f9f9;">
                        📧 Send email
                    </button>
                </div>
